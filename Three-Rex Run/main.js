...
class BasicWorldDemo {
  ...
  _Initialize() {
    ...
    // nightButton=document.getElementById("nightButton");
    // nightButton.addEventListener('click',function(){
    //   console.log("Night is selected");
    //   gameMood=0;
    //   console.log(gameMood);
    // });
    ...
    if(gameMood)
    {
      this.camera_.position.set(-5, 5, 10); //day camera setup
    }else{
      this.camera_.position.set(-5, 5, -10); // night camera setup
    }
    ...
    if(gameMood)
    {
      light = new THREE.DirectionalLight(0xd58253, 1.0);  //0xD2B48C   0xFFFFFF
    }else{
      light = new THREE.DirectionalLight( 0x5C2D19, 1.0); // Desert color
    }
    ...
    // I put an condition to view the day and the night 
    ...
    {
    skyMat = new THREE.MeshBasicMaterial({ color: 0x010101, side: THREE.BackSide });
    this.scene_.add(new THREE.Mesh(skyGeo, skyMat));
    }


    // const skyGeo = new THREE.SphereBufferGeometry(1000, 32, 15);     //Sky color
    // const skyMat = new THREE.MeshBasicMaterial({ color: 0x010101, side: THREE.BackSide });
  ...
  Step_(timeElapsed) {
    ...
    if (this.player_.gameOver && !this.gameOver_) {
      ...
      this.gameOver_ = true;  //Game over

      yourScore=Number(document.getElementById('score-text').innerText);
      if(localStorage.getItem('highestScore') !== null){
        prevScore=Number(localStorage.getItem('highestScore'));
      ...
      document.getElementById('prvHighestScore').innerHTML=`Highest Score ${localStorage.getItem('highestScore')}`;
      document.getElementById('yourCurrentScore').innerHTML=`Your Score ${yourScore}`;
